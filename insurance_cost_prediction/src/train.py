...
run the following first, because we are now working on google colab and google drive. have to run the following so that we can use `from preprocessing import...` (otherwise the python in google colab dunno whr to look for preprocessing.py)
"""
...
# IMPORTANT: Change this to the exact path where your preprocess.py file is saved
...
# Assuming 'preprocessor' is an existing ColumnTransformer object

# Separate features (X) and target (y) from the original DataFrame
# Assuming 'charges' is the target column name
try:
    X = df.drop('charges', axis=1)
    y = df['charges']
except NameError:
...
# Define the PCA step
# You can choose the number of components based on your PCA analysis (e.g., from the explained variance plot)
# For example, to keep 95% variance:
n_components_to_keep = 10 # Example: choose a number of components based on your analysis
...
# Create the full pipeline including preprocessor, PCA, and Linear Regression
try:
...
*   **Age:** Associated with **Higher Charges** (primarily through PC1).
...
*   **Being Obese/Higher BMI:** Associated with **Higher Charges** (primarily through PC2 and PC9).
*   **Northwest Region:** Associated with **Lower Charges** (through PC5).
*   **Southwest Region:** Associated with **Higher Charges** (through PC5).
...
# Assuming lasso_pipeline is available from cell wn6HaM2BuI5x (Lasso on original features)
# Assuming linear_reg_pca_pipeline is available from cell d668fa41 (Linear Regression with PCA)
# Assuming best_xgb_pipeline is available from cell IJMiCFbwvZNS (XGBoost with PCA)
# Assuming project_path is defined from cell VyDxnTEwjm0z

# Save the Lasso pipeline (on original features)
lasso_model_filename = 'lasso_pipeline.joblib'
lasso_model_save_path = project_path + lasso_model_filename
...
linear_reg_pca_model_save_path = project_path + linear_reg_pca_model_filename
...
random_search.fit(X_train, y_train_log, model__sample_weight=weights_train)
...
model_save_path = project_path + final_model_filename
